...
public class TradeCommand {
    ...
    private static int requestTrade(CommandSourceStack source, ServerPlayer target) {
        ...
        // Distance Check
        double maxDist = TradeConfig.MAX_TRADE_DISTANCE.get();
        if (maxDist > 0) {
        ...
        if (targetPending != null) {
            if (now > targetPending.expirationTime) {
                ...
                long cooldownTime = TradeConfig.TRADE_COOLDOWN_SECONDS.get() * 1000L;
                if (cooldownTime > 0) {
        ...
        // Create new request
        long expireAt = now + (TradeConfig.REQUEST_TIMEOUT_SECONDS.get() * 1000L);
