...
public final class TradeRequestManager {
    ...
    private static final class CooldownKey {
        ...
        public boolean equals(Object other) {
            ...
            if (!(other instanceof CooldownKey key)) {
                return false;
            }
    ...
    private static final Map<UUID, Request> requestsByParticipant = new HashMap<>();
    private static final Map<CooldownKey, Long> cooldowns = new HashMap<>();

    private TradeRequestManager() {
    }

    public static synchronized CreateResult create(
            UUID senderId,
            UUID targetId,
            long now,
            long timeoutMillis,
        ...
        if (cooldownEnd != null && cooldownEnd > now) {
            int seconds = (int) Math.ceil((cooldownEnd - now) / 1000.0);
    ...
    public static synchronized Request takeIncoming(UUID targetId, long now, long cooldownMillis) {
        ...
        if (request == null || !request.targetId.equals(targetId)) {
            return null;
        }
        remove(request);
        return request;
    ...
    public static synchronized void deny(Request request, long now, long cooldownMillis) {
        remove(request);
        if (cooldownMillis > 0) {
            cooldowns.put(new CooldownKey(request.senderId, request.targetId), now + cooldownMillis);
        }
    }
    ...
    public static synchronized void prune(long now, long cooldownMillis) {
        Request[] requests = requestsByParticipant.values().stream().distinct().toArray(Request[]::new);
        for (Request request : requests) {
            if (request.expirationTime < now) {
                remove(request);
                if (cooldownMillis > 0) {
                    cooldowns.put(
                            new CooldownKey(request.senderId, request.targetId),
                            request.expirationTime + cooldownMillis
                    );
                }
            }
        }
